...
namespace ChatServer.Service
{
    public class HubService : IHubService
    {
        ...
        public HubService(IUserRepository userRepository, IChatRepository chatRepository
            , IApplicationRepository applicationRepository, IUserProfileService userProfileService){
            ...
            if (app != null)
            {
                ...
                if (chat != null && user != null)
                {
                    if (chat.ChatMembers.Any(member => member.UserId == user.Id))
                    {
                        ...
                        return new UserChatVM(){UserId=user.Id
                            , Chat = new Chat { Id = chatId,Name = chat.Name, ChatConversations = lastChatConversations, ChatType=chat.ChatType }, NewMessagesCount=0};
                    }
                }
            }
            return null;
